...
def learn(bool_evaluate, robot, policy_func, dataset, optim_batch_size=64, max_iters=5*1e3,
    ...
    if bool_evaluate:
        saver.restore(tf.get_default_session(), 'model/bc.ckpt')
        return pi
...
def main(args):
    ...
    if args.task == 'train':
        reward_giver = Adversary(ob_shape, ac_shape, args.adversary_hidden_size)
        train(robot,
              args.seed,
              policy_fn,
              reward_giver,
